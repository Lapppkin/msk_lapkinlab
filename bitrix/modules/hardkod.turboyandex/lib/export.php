...
if (!function_exists("yandex_text2xml_text"))
{
	function yandex_text2xml_text($text, $bHSC = false, $bDblQuote = false)
	{
		...
		$text = preg_replace_callback("/&([A-Za-z]*){2,6};/Us", function($matches) {
			switch ($matches[1]) {
				case 'gt':
					return '&amp; qt;';
				case 'lt':
...
class RssExport
{
    ...
    private $file;
	public $statistic;
    private $allowed_tags = '<a><figure><div><h2><h3><h4><span><h5><h6><p><br><ul><ol><li><b><strong><i><em><sup><sub><ins><del><small><big><pre><abbr><u><img><figcaption><video><figure><header>';
    ...
    private function format_item($data) {
        if (in_array($data['link'], $this->pagecodes)) {
			return false;
		}
		
        $data['content'] = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $data['content']);
        $data['content'] = preg_replace('#<style(.*?)>(.*?)</style>#is', '', $data['content']);
        ...
        $data['content'] = preg_replace('/(<[^>]+) class=".*?"/i', '$1', $data['content']);
        $data['content'] = preg_replace('/(<[^>]+) align=".*?"/i', '$1', $data['content']);
		
		$data['content'] = preg_replace_callback("/href=\"#([^\"]*)\"/is", function($matches) use ($absolute_url) {
			$url = preg_replace("/^\/(.*)/", "\\1", $matches[1]);
		...
		if (mb_strlen($data['title']) > 220) {
			$data['title'] = mb_substr($data['title'], 0, 220);
		}
        ...
        if($data['title'] && $data['content']) {
            ...
            if($data['author'] && Option::get(ADMIN_MODULE_NAME, "export_author")) {
		...
		if (strlen($content_text) > 350) {
			...
			foreach ($content as $line_key => $line_val) {
				if (strlen($line_val) > 350) {
					...
					if (!preg_match("/(\<p|\<br|\<ol|\<ul|\<li|\<div)/is", $line_val)) {
						//$content[ $line_key ] .= '<br>';
					}
				}
			}
