  ...
  $(function(){
    ...
    onChange: function (data) {
      $("#range2").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_2");
           if(data.from == '0'){$result.html("0-20" );}
           if(data.from == '1'){$result.html("21-50" );}
           if(data.from == '2'){$result.html("51-100" );}
           if(data.from == '3'){$result.html("101-250" );}
           if(data.from == '4'){$result.html("250-1000" );}
    }
    });
    ...
    onChange: function (data) {
      $("#range3").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_3");
           if(data.from == '0'){$result.html("1-10" );}
           if(data.from == '1'){$result.html("11-20" );}
           if(data.from == '2'){$result.html("21-50" );}
           if(data.from == '3'){$result.html("51-150" );}
           if(data.from == '4'){$result.html("150+" );}
    }
    });
    ...
    onChange: function (data) {
      $("#range4").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_4");
           if(data.from == '0'){$result.html("Москва" );}
           if(data.from == '1'){$result.html("Санкт-Петербург" );}
           if(data.from == '2'){$result.html("Краснодар" );}
           if(data.from == '3'){$result.html("Город-миллионер (кроме мск и спб)" );}
           if(data.from == '4'){$result.html("Город с населением менее 1 млн. человек" );}
           if(data.from == '5'){$result.html("Более 2-х городов" );}
           if(data.from == '6'){$result.html("Более 4-х городов" );}
    }
    });
    ...
    from: 0,
    values: [
        "Практически нет конкурентов", "Низкий",
        "Средний", "Высокий",
        "Яблоку негде упасть"
    ],
    onChange: function (data) {
      $("#range5").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_5");
           if(data.from == '0'){$result.html("Практически нет конкурентов" );}
           if(data.from == '1'){$result.html("Низкий" );}
           if(data.from == '2'){$result.html("Средний" );}
           if(data.from == '3'){$result.html("Высокий" );}
           if(data.from == '4'){$result.html("Яблоку негде упасть" );}
    }
    });
    ...
    from: 0,
    values: [
        "Сейчас продвигается", "Качественно продвигался ранее",
    ...
    onChange: function (data) {
      $("#range16").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_6");
           if(data.from == '0'){$result.html("Сейчас продвигается" );}
           if(data.from == '1'){$result.html("Качественно продвигался ранее" );}
           if(data.from == '2'){$result.html("Мало / практически не продвигался" );}
           if(data.from == '3'){$result.html("Не продвигался" );}
           if(data.from == '4'){$result.html("Новый сайт" );}
    }
    });
    ...
    from: 0,
    values: [
        "Сайт не адаптивный", "Сайт адаптивный"
    ],
    onChange: function (data) {
      $("#range10").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_11");
           if(data.from == '0'){$result.html("Сайт не адаптивный" );}
           if(data.from == '1'){$result.html("Сайт адаптивный" );}
    }
    });
    ...
    onChange: function (data) {
      $("#range13").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_12");
           if(data.from == '0'){$result.html("Нет" );}
    ...
    onChange: function (data) {
      $("#range8").data("ionRangeSlider").update({"disable": false});
          var $result = $("#result_7");
           if(data.from == '0'){$result.html("Не требуется" );}
           if(data.from == '1'){$result.html("Настройка" );}
           if(data.from == '2'){$result.html("Ведение" );}
           if(data.from == '3'){$result.html("Настройка и ведение" );}
    }
    });
  });
...
document.getElementById("foocalc").value+="\nКОЛИЧЕСТВО РАЗДЕЛОВ САЙТА: "+r2.result.from_value;
           ...
           if(r3.result.from == '5'){r3sum=2;}
           if(r3.result.from == '6'){r3sum=4;}
...
$("#result_itogo").html(sum);
      return false;
       ...
       $("A#trigger").click(function() {
        ...
        }else{alert("Заполните корректно все поля!");var scrollTop = $('#cccallk').offset().top;$(document).scrollTop(scrollTop);}
       });
    });